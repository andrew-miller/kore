...
public class RVertexComparer implements Comparer<RVertex> {
  public Comparison compare(RVertex a, RVertex b) {
    ...
    if (x != Comparison.EQ) return x;
    switch (a.tag) {
      case UNION:
        return new RVUnionComparer().compare(a.union(), b.union());
      case ABSTRACTION:
        return new RVAbstractionComparer().compare(a.abstraction(), b.abstraction());
      case COMPOSITION:
        return new RVCompositionComparer().compare(a.composition(), b.composition());
      case LABEL:
